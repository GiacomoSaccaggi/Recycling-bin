...
Il programma si è deciso di chiamarlo \virgolette{Automatic Recycling bin} ed è strutturato in 3 pagine.
...
Infatti, una rete neurale feed-forward, può essere considerata come una composizione di funzioni e se tutte queste risultano lineari allora anche la funzione composta sarà lineare.
$$\b x_0
\longrightarrow
...
\begin{figure}[H]
	...
	\caption{Matrice di confusione per problema di classificazione multi-class}
\end{figure}
...
Durante l'implementazione dei modelli del capitolo precedente si è ritenuto interessante anche valutare come questi possano essere combinati per migliorare le previsione.
Questo ultimo capitolo si vuole, appunto, focalizzare sull'armonizzazione al fine di aumentare l'accuratezza di previsione dei modelli con lo scopo di raggiungere due obbiettivi:

\begin{itemize}
	\item Creare un'algoritmo di classificazione che possa unire gli algoritmi ResNet addestrati su immagini per implementare una applicazione che possa dalla foto del rifiuto individuare la classe corrispondente ad esso.
	\item Costruire un metodo che possa combinare informazioni derivanti da dati strutturati e da foto per riuscire a sfruttare tutte le informazioni ricavate da questo studio, allo scopo della creazione (anche se solo in fase progettuale) di un cestino che sia in grado di riconoscere e dividere i rifiuti.
\end{itemize}

La scelta operativa, come spiegato nel Capitolo 2.1, è stata quella di creare un Validation Set che contenga le informazioni strutturate e non strutturate così da ottenere un dataset comune, necessario per la valutazione ed il confronto delle performance.

L'obbiettivo è quello di valutare la reale validità degli algoritmi proposti, in tal senso per non agevolare la previsione a nessun metodo addestrato si è deciso di valutarli in condizioni ambientali differenti cambiando la posizione del cestino così da tentare di valutare e correggere quanto le informazioni prese ed analizzate dal GBM e dalla MLP fossero dipendenti dall'ambiente esterno.
...
Un ulteriore scelta è stata quella di inserire in sequenza casuale foto relative all'oggetto catturate dal cestino o foto fatte in ambienti esterni così da valutare come le ResNet si comportassero in ambienti differenti. 



Una volta valutate le prestazioni dei singoli metodi sarà l'obbiettivo delle sezioni seguenti fondere questi modelli per ottenere un accuratezza maggiore.

$$ $$

\begin{table}[h!]
	\begin{center}
		\caption{Dati strutturati}
		\label{tab:totstruct}
		\vspace{0.5cm}
...
Dai risultati sul nuovo Validation Set si può notare come ci sia un abbassamento dell'accuratezza globale dovuta al cambio di ambiente. Focalizzando l'attenzione sui modello creati sui dati strutturati si può osservare, in Tabella 3.1, come la MLP non riesca a prevedere in modo accurato. Questo problema di overfitting che porta ad un abbassamento dell'accuratezza di 32 punti percentuale ha portato all'eliminazione di questa previsione per evitare di inficiare sugli altri modelli inserendo possibili caratteri distorsivi. Anche se non in modo così ampio, anche il Gradient Boosting Algorithm non reagisce in modo flessibile al cambiamento dell'ambiente infatti l'accuratezza cala fino a 0.71.

Le ResNet addestrate sui dati non strutturati reagiscono in modo differente. Come era preventivabile il Modello addestrato sulle foto raccolte dal WEB ha un abbassamento dell'accuratezza di previsione importante dovuto alla poca specificità delle foto; mentre la ResNet addestrata sulle foto fatte ad hoc per il progetto reagisce in modo molto accurato anche oltre le aspettative. Dalla performance di questa ultima rete si può intuire quanto queste tipologie di architetture siano potenti anche con una quantità più ridotte di dati.
...
I metodi di Ensemble Learning vengono comunemente utilizzati per migliorare la precisione predittiva combinando le previsioni di più modelli di apprendimento automatico. Quando questi metodi furono ideati a metà degli anni '90 del secolo scorso, l'idea era quella di combinare i cosiddetti predittori ``deboli" (Breiman Leo; 1996)~\cite{chiave16}. Tuttavia, un approccio più moderno è quello di creare un insieme di una collezione ben scelta di modelli forti ma diversi (Van der Laan, Polley e Hubbard; 2007)~\cite{chiave17}.


Con tutte le differenze del caso, la costruzione di potenti modelli di Ensemble Learning ha molti parallelismi con la creazione di team di persone esperte in diversi settori (nel mondo degli affari, della scienza, della politica, dello sport, ecc.). Nel cooperare per un progetto ogni membro del team fornisce un contributo significativo e le debolezze e le distorsioni individuali vengono compensate dai punti di forza degli altri membri, così è per i modelli di Ensemble Learning che sfruttano Stronger Learner.

La tipologia più semplice per fondere molteplici metodi consiste nella media non ponderata delle previsioni dei modelli che formano una serie di strumenti di indagine. Nella costruzione di un modello di Ensemble Learning con una media non ponderata, ogni modello assume lo stesso peso.

Più in generale, si può pensare di utilizzare le medie ponderate così da poter conferire più peso a quei modelli migliori. Quando le informazioni sul fenomeno oggetto di analisi sono molto abbondanti si può scegliere di assegnare direttamente e soggettivamente i pesi ai modelli. Un approccio più performante potrebbe essere quello di stimare i pesi usando un altro livello di stima attraverso un algoritmo di classificazione. Quest'ultimo approccio proposto è chiamato Stacking dei modelli.

Lo Stacking dei modelli~\cite{chiave18} è un metodo di Ensemble Learning efficiente in cui le previsioni, generate utilizzando algoritmi di Machine Learning, vengono utilizzate come input in un modello di apprendimento di secondo livello. Questo algoritmo di secondo livello viene addestrato per combinare in modo ottimale le previsioni formando una nuova serie di previsioni. 

Si parla di regressione Stacking quando si utilizza la regressione lineare come algoritmo di modellazione di secondo livello. In questo contesto i pesi vengono trovati minimizzando gli errori quadrati tra le previsioni di primo livello e i valori obbiettivo. 
...
L'algoritmo per creare un modello a più livelli con il metodo Stacking si può generalizzare a qualsiasi metodo di apprendimento automatico dividendo il suo processo di costruzione in tre fasi:


\begin{itemize}
	\item Impostare l'Ensemble Learning:
	\begin{itemize}
	...
	\end{itemize}
	\item Creare l'Ensemble Learning:
	\begin{itemize}
	\item Addestrare ciascuno degli $L$ Base Learner sul training set $Y=F(X)$ e trovare delle previsioni $Z$ di primo livelllo.
	\item Eseguire il training del meta-algoritmo di apprendimento sui dati di livello 1 $Y=F(Z) \mbox{ con } Z=F(X)$. Il ``modello di Ensemble Learning", quindi, sarà costituito dagli $L$ modelli di apprendimento di base e il modello di Stacking che combina i modelli precedenti.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Si inseriscono tali previsioni nello modello di Ensemble Learning per generare la previsione dell'insieme.
	\end{itemize}
\end{itemize}

In alcuni ambiti con molte informazioni circa la variabile di studio spesso viene utilizzato un modello di Stacking a più di due livelli, in questi casi occorre estendere la metodologia proposta reiterandola per il numero dei livelli.

Per comprendere come questi modelli possano essere sfruttati si propone come esempio il modello vincitore della KDD Cup 2015 creato da Jeong-Yoon Lee. In questa competizione i dati sono stati forniti da due piattaforme online di apprendimento XuetangX e Chinese MOOC. L'obbiettivo dell'analisi consisteva nel cercare di prevedere l'abbandono del corso durante i primi 10 giorni dall'iscrizione. L'approccio vincente ha utilizzato un modello di Ensemble Learning a tre stadi.



\begin{figure}[H]
	\center
	\includegraphics[width=\textwidth]{stackedapproach.png}
	\caption{Jeong-Yoon Lee - KDD Cup 2015}
\end{figure}

La Figura 3.1 mostra che un set diversificato di 64 modelli singoli sono stati utilizzati per compilare la libreria di modelli. Questi modelli vengono sottoposti a training usando vari algoritmi di apprendimento automatico. Le caselle verdi rappresentano i modelli di Bosting (GBM) che probabilmente variano nell'uso di diverse impostazioni di iperparametri e/o set di funzionalità, le caselle rosa rappresentano delle reti neurali (NN) e le caselle arancioni rappresentano i modelli Factorization Machines (FM)~\cite{chiave19}. 

Nella fase 1, le stime di questi 64 modelli vengono usate come input per eseguire il training di 15 nuovi modelli, sempre usando vari algoritmi di apprendimento automatico. Nella fase 2, le stime di questi 15 modelli vengono utilizzate come input per eseguire il training di due modelli utilizzando il Gradient Boosting e la regressione lineare. Nella fase 3 le previsioni dei due modelli dalla fase 2 vengono utilizzate come input in un modello di regressione Stacking per formare la previsione finale.

L'overfitting è un problema particolarmente importante nello Stacking dei modelli, perché vengono combinati tanti predittori che stimano tutti lo stesso target. 
Una possibile soluzione a questo problema è quella di generare delle serie diversificate di modelli utilizzando vari metodi con una logica alla base differente (come alberi decisionali, reti neurali e/o altri metodi). Un'altra possibile soluzione consiste nell'utilizzare diversi tipi di dati così da cercare di inserire nel modello finale informazioni nuove non presenti nei modelli precedenti.
...
Naturalmente, questo metodo comporta costi computazionali aggiuntivi sia perché è necessario addestrare un numero elevato di modelli sia perché spesso risulta necessario utilizzare la convalida incrociata per evitare un eccesso di adattamento. 


\subsection{Risultati}

Il meta-algoritmo scelto in questa parte del progetto è il K-nearest neighbors (KNN)~\cite{chiave20}, questo può essere utilizzato sia per problemi predittivi di classificazione (binaria e multi-class) che di regressione. Tuttavia, è comunemente utilizzato nei problemi di classificazione per il suo approccio semplice che gli permette di non fare assunzioni circa la struttura delle variabili.



L'intuizione alla base dell'algoritmo KNN è quella di calcolare la distanza di un nuovo punto da tutti gli altri punti nel training e selezionare quindi i punti $K$ più vicini al punto obbiettivo. Infine l'algoritmo assegna alla nuova rilevazione la classe a cui appartiene la maggior parte dei punti compresi nell'intervallo dei $K$ più vicini.
...
\begin{itemize}
	\item Le misure di distanza possono essere di qualsiasi tipo anche se solitamente si utilizzano quella Euclidea, di Manhattan o di Minkowski con $p>2$.
	\item Il parametro $K$ deve essere un numero intero positivo. Questo valore può essere scelto a priori in modo arbitrario oppure attraverso tecniche di validazione come Cross Validation.
\end{itemize}
 
$$ $$
 
Per verificare la validità di questo modello si esegue un semplice esempio nel quale si suppone di disporre di un set di dati con due variabili che, se tracciato, è simile a quello in Figura 3.2. Lo scopo è quello di classificare un nuovo punto rappresentato da ``$X$" in classe ``$Verde$" o ``$Rosso$". Si ipotizza di fissare il valore di $K$ uguale a 3.
L'algoritmo KNN inizia calcolando la distanza del punto X da tutti i punti e, come mostrato in Figura 3.2 nell'immagine di sinistra, trova i $K=3$ punti più vicini con distanza minima rispetto al punto X.



\begin{figure}[H]
	\center
	\includegraphics[width=\textwidth]{KNN.jpg}
	\caption{Esempio K-nearest neighbors (KNN)}
\end{figure}

Il passaggio finale dell'algoritmo KNN consiste nell'assegnare al nuovo punto $X$ la classe a cui appartiene la maggior parte dei $K=3$ punti più vicini. Dalla figura precedente possiamo vedere che due dei tre punti più vicini appartengono alla classe ``$Verde$" mentre uno appartiene alla classe ``$Rosso$". Pertanto $X$ verrà classificato come ``$Verde$". 
Il procedimento appena enunciato viene reiterato per ogni valore dello spazio dei punti, come mostrato in Figura 3.2 nell'immagine di destra, così da riuscire a classificare operando una divisione del piano.

\clearpage

Per costruire il modello si è diviso il set in train e test data. Successivamente si è utilizzata una k-fold cross validation con $k=5$ per stimare il parametro $K$ dell'algoritmo sul training set che poi è stato utilizzato per stimare i valori di test. Come misura di distanza si è utilizzatala ``distanza di Euclidea" la quale deriva dalla ``distanza di Minkowski" con $p=2$. Per una serie di punti n-dimensionali $P = \{\underline p_{1}, ... ,\underline p_{m}\} $ rispetto ad un punto $\underline q$, questa distanza risulta essere:



$$\mbox{Distanza di Minkowski}_j = \bigg( \sum_{i=1}^{n} |p_{i,j} - q_i |^p\bigg)^{\frac{1}{p}} \space \space \mbox{ con } j=1,...,m$$


$$ $$

$$\mbox{Distanza Euclidea}_j = \sqrt{ \sum_{i=1}^{n} (p_{i,j} - q_x )^2} \space \space \mbox{ con } j=1,...,m $$



$$ $$ 
Il \textbf{primo modello} creato utilizza come modelli di primo livello le ResNet create nel capitolo precedente le quali vengono combinate dal meta algoritmo appena spiegato.
$$ $$
...
\begin{table}[h!]
	\begin{center}
		\caption{Confusion matrix relativa al modello di Ensemble Learning tra ResNet}
		\label{tab:EL1a}
		\vspace{0.5cm}
...
\begin{table}[h!]
	\begin{center}
		\caption{Classification Report relativo al modello di Ensemble Learning tra ResNet}
		\label{tab:EL1b}
		\vspace{0.5cm}
...
Come si può notare l'accuratezza migliora notevolmente con l'utilizzo del modello di Stacking. Le due reti neurali convoluzionali riescono ad essere armonizzate dal K-nearest neighbors riuscendo a raggiungere un accuratezza superiore al 90\%. 



$$ $$ 
Il \textbf{secondo modello} utilizza come informazione aggiuntiva rispetto a quello appena descritto le previsioni del modello GBM costruito sui dati strutturati.
$$ $$


\textbf{Modello: Ensemble Learning tra ResNet e GBM}
...
\begin{table}[h!]
	\begin{center}
		\caption{Confusion matrix relativa al modello di Ensemble Learning tra ResNet e GBM}
		\label{tab:EL2a}
		\vspace{0.5cm}
...
\begin{table}[h!]
	\begin{center}
		\caption{Classification Report relativo al modello di Ensemble Learning tra ResNet e GBM}
		\label{tab:EL2b}
		\vspace{0.5cm}
...
$$ $$
L'inserimento nel modello di Ensemble Learning delle informazioni strutturate rende le previsioni più precise consentendo di raggiungere un accuratezza del 95\%. C'è un miglioramento globale anche per quanto riguarda la ``Precision" e il ``Recall" come si può notare dalle medie delle misure ``macro avg" e ``weighted avg".

$$ $$
I modelli addestrati in questo capitolo attraverso metodi di Stacking hanno il vantaggio avere delle stime più robuste e più accurate rispetto a quelle del capitolo precedente. Il K-nearest neighbors riesce, quindi, ad essere un ottima scelta per la costruzione di modelli di secondo livello. 

La natura differente dei modelli e la flessibilità del meta-algoritmo scelto permette al modello di essere ragionevolmente più generalizzabile fuori dal test set. I problemi legati all'overfit, per quanto possibile in un problema di previsione, non aumentano rispetto ai singoli modelli in quanto le due architetture utilizzano immagini di natura differente inoltre, per evitare problemi di adattamento, il GBM utilizza informazioni non ricavabili in alcun modo dai modelli precedenti. 
...
La prima applicazione che si è presa in esame è l'implementare di un Chatbot. Il software che si è utilizzato come piattaforma è l'applicazione di messaggistica, Telegram. La realizzazione consiste in un programma che è in grado di riceve in input delle foto e di restituire come output la categoria di rifiuto corrispondente dell'oggetto presente nell'immagine. Per implementare questa idea è stato necessario disporre un server che fosse in grado di ricevere la foto e, utilizzando il primo algoritmo di Ensemble Learning creato nella sezione precedente, fosse in grado di analizzarla e restituire nella chat la categoria di rifiuto predetta.

\begin{figure}[H]
	\center
	\includegraphics[width=150px]{bot_riconoscimento.jpg}
	\caption{Bot di Telegram}
\end{figure}

Nella programmazione di questo Bot, come per quello relativo al Capitolo 1.4, si è utilizzata la libreria di Python ``Telepot" grazie alla quale si possono inviare e riceve (GET e POST request nel linguaggio PHP) richieste al server direttamente dalla chat. 

Il primo step del lavoro è stato quello di salvare i modelli preaddestrati in formato ``pickle" e ``hdf5" così da poterli importare rapidamente per l'analisi. Le ResNet sono state entrambe addestrate sottraendo la media dei rispettivi training set così da avvantaggiare la previsione sui caratteri distintivi. La media di più immagini contiene tutti i caratteri in comune presenti nelle foto, ovvero ciò che comunemente viene definito ``sfondo". Le medie trovate dopo essere state salvate in fase di addestramento vengono utilizzate per ogni previsione sottraendo alla matrice dei pixel della nuova immagine tali medie.

Le nuove immagini da analizzare vengono importate operando un ``resize" per trasformarle in una matrice 32x32 pixel. Al fine di programmare la classificazione è necessario, dunque, strutturare attraverso una funzione $previsione()$ i due livelli di Stacking:


\begin{itemize}
	\item Primo livello: 
	
	Utilizzo dei due modelli ResNet per classificare la nuova immagine. Tramite questi vengono stabiliti dei pesi che devono essere trasformati nella medesima forma di come sono state inserite nel modello di Ensemble Learning.
	
	\item Secondo livello: 
	
	Utilizzo dell'algoritmo KNN per la costruzione della stima finale tramite i pesi trovati nel primo livello.
\end{itemize}
...
\caption{Script di programmazione 3.1: Funzione di previsione}
\tiny
\begin{lstlisting}[language=Python]

def previsione(img_path):
	img_data = []
	image = cv2.imread(img_path)
	
	# standard imm
	resized_img = cv2.resize(image, (32, 32), interpolation=cv2.INTER_AREA)
	img_data.append(resized_img)
	img_data=np.array(img_data)
	img_data = img_data.astype('float32') / 255
	img_data_cestino = img_data - cestino_mean
	img_data_WS = img_data - ws_mean
	
	# Prediction
	## resnet1
	# categories=['Carta', 'Indifferenziato', 'Latta', 'Legno', 'Metalli', 'Plastica', 'Polistirolo', 'Tetra_Pak', 'Umido', 'Vetro']
	pred2 = resnetWS.predict(img_data_WS)
	for t in range(len(pred2[0])):
	pred2[0][t]=round(pred2[0][t],3)*1000
	pred = list(pred2[0])
	## resnet2
	# categories=['carta','indifferenziata',  'plastica', 'vetro']
	pred1 = resnetCestino.predict(img_data_cestino)
	for t in range(len(pred1[0])):
	pred1[0][t]=round(pred1[0][t],3)*1000
	pred.append(list(pred1[0])[0])
	pred.append(list(pred1[0])[1])
	pred.append(list(pred1[0])[2])
	pred.append(list(pred1[0])[3])
	X = pd.DataFrame(columns=colonne)
	X.loc[0]=pred
	predictions = knn_gscv.predict(X)
	return predictions

\end{lstlisting}
\normalsize

La classe corrispondente al rifiuto viene poi restituita al mittente della foto, identificato da $chat\_id$, tramite la funzione $.sendMessage()$.


\vspace{0.6cm}

 \caption{Script di programmazione 3.2: Funzione di rispondi}
\tiny
\begin{lstlisting}[language=Python]
...
def rispondi(msg):
	...
	if content_type == 'photo':
		...
		risultato = previsione('image.png')
		bot.sendMessage(chat_id, str(risultato[0]))
\end{lstlisting}
\normalsize
...
\clearpage
\subsection{Sistema automatico di raccolta differenziata}

La seconda applicazione che si è presa in esame consiste in un cestino in grado di fare la raccolta differenziata in automatico. Ai fini della lettura e della comprensione del progetto descritto è utile considerare che in questa sezione si andrà a sviluppare la parte statistica di previsione e la parte softwaristica; mentre la parte di robotica verrà implementata solo in fase progettuale.

Nel Capitolo 1.5 si è costruito un cestino in grado di registrare i dati sui rifiuti. Utilizzando la struttura del cestino, già spiegata in precedenza, si è implementata una terza pagina del software, denominato \virgolette{Automatic Recycling bin}, che automatizza la fase di riconoscimento dei rifiuti e divisione nei differenti scomparti.

La terza pagina del programma, indicizzata come \virgolette{Form3: Riconoscimento}, occorre che sia inizializzata collegando la fotocamera e la porta seriale al software. Cliccando sul pulsante \virgolette{Button1: Riconosci}, dopo aver inserito l'oggetto nell'apposito scompartimento, il cestino sarà in grado, attraverso le funzioni programmate appositamente, di prevedere la categoria che corrisponde al rifiuto. L'output della previsione verrà inviato al programma e inserito nella TextBox posta sopra il pulsante.

Il programma è stato creato in VB.net, le previsioni vengono stimate in Python e il cestino è stato programmato in C++ quindi era importante riuscire a creare un processo con il quale questi tre linguaggi potessero comunicare tra loro. 

Tra le possibili opzioni si è deciso di installare all'interno dell'applicazione una versione ``Portable" di Python con la quale si esegue lo script di previsione all'interno del codice di VB.net, richiamando l'interprete attraverso l'utilizzo del terminale di Windows.

Gli script di programmazione del software sono presenti nell'Appendice A.3.3. Di seguito, si elencano gli step di implementazione della funzione inserita all'interno del pulsante \virgolette{Button1: Riconosci}:

\begin{itemize}
	\item [1.] Registrazione dei dati strutturati;
	\item [2.] Fotografia dell'oggetto inserito all'interno del cestino;
	\item [3.] Script ``analizza.py". Attraverso un processo in background viene lanciato lo script dal terminale inserendo come argomenti i parametri registrati dal software:
	\begin{itemize}
		\item [a.] Calcolo dei modelli di primo livello;
		\item [b.] Utilizzo del secondo modello di Ensemble Learning per il calcolo delle previsioni finali;
		\item [c.] Previsione finale salvata in un documento di testo all'interno della directory dell'applicazione.
	\end{itemize}
	\item [4.] Notifica all'utente del processo in background tramite una Progress Bar;
	\item [5.] Lettura del file creato con la previsione al punto 3.c.
	\item [6.] Eliminazione automatica del file
	\item [7.] Invio all'utente del risultato della previsione tramite una TextBox posizionata al centro della finestra;
	\item [8.] Scrittura sulla porta seriale di un codice identificativo, differente in base alla previsione, che permette di attivare una funzione all'interno dell'Arduino che istruisce il cestino su quanto deve ruotare e in quale scompartimento deve posizionare il rifiuto.
\end{itemize}
...
\begin{figure}[H]
	\center
	\includegraphics[width=300px]{form3nuova.png}
	\caption{Form3: Riconoscimento}
\end{figure}
...
I modelli creati, come è possibile osservare in Figura 2, hanno un accuratezza elevata. In particolare si può notare come i modelli di Stacking creati hanno un accuratezza superiore al 90\% ed una precisione nelle singole categorie di rifiuto altrettanto elevata.


\begin{figure}[H]
	\center
	\includegraphics[width=\textwidth]{accuratezza.png}
	$$ $$
	Figura 2: Risultati dei modelli
\end{figure}
...
\begin{itemize}
	\item L'utilizzo del Reinforced Learning per consentire alle applicazioni di apprendere dagli errori.
	\item Lo sfruttamento, congiuntamente ai classificatori creati, degli strumenti di Amazon Web Services per creare nuovi modelli di Ensemble Learning più accurati.
	\item La lettura, quando presente nell'immagine, del bar code del prodotto da smaltire. 
\end{itemize}	
...
img<-c("nome1","nome2") #array di nomi per scaricare le immagini inerenti ad un argomento
...
void loop() {
	...
	luce1=analogRead(A2);
	luce2=analogRead(A3);
	unsigned int distanza1a = sonar1.ping();
	distanza1a=distanza1a / US_ROUNDTRIP_CM;
	unsigned int distanza1b = sonar1.ping();
	distanza1b=distanza1b / US_ROUNDTRIP_CM;
	unsigned int distanza1c = sonar1.ping();
	distanza1c=distanza1c / US_ROUNDTRIP_CM;
	unsigned int distanza1d = sonar1.ping();
	distanza1d=distanza1d / US_ROUNDTRIP_CM;
	unsigned int distanza1e = sonar1.ping();
	distanza1e=distanza1e / US_ROUNDTRIP_CM;
	unsigned int distanza1f = sonar1.ping();
	distanza1f=distanza1f / US_ROUNDTRIP_CM;
	unsigned int distanza1g = sonar1.ping();
	distanza1g=distanza1g / US_ROUNDTRIP_CM;
	int distanza1 = (distanza1a+distanza1b+distanza1c+distanza1d+distanza1e+distanza1f+distanza1g)/7;
	int vardistanza1 = ((distanza1-distanza1a)^2+(distanza1-distanza1b)^2+ (distanza1-distanza1c)^2+(distanza1-distanza1d)^2+ (distanza1-distanza1e)^2+(distanza1-distanza1f)^2+ (distanza1-distanza1g)^2)/7;
	digitalWrite(motorpinDC,LOW);
	magnetic=analogRead(A0);
	luce2=analogRead(A2);
	luce2=analogRead(A3);
	unsigned int distanza2a = sonar2.ping();
	distanza2a=distanza2a / US_ROUNDTRIP_CM;
	unsigned int distanza2b = sonar2.ping();
	distanza2b=distanza2b / US_ROUNDTRIP_CM;
	unsigned int distanza2c = sonar2.ping();
	distanza2c=distanza2c / US_ROUNDTRIP_CM;
	unsigned int distanza2d = sonar2.ping();
	distanza2d=distanza2d / US_ROUNDTRIP_CM;
	unsigned int distanza2e = sonar2.ping();
	distanza2e=distanza2e / US_ROUNDTRIP_CM;
	unsigned int distanza2f = sonar2.ping();
	distanza2f=distanza2f / US_ROUNDTRIP_CM;
	unsigned int distanza2g = sonar2.ping();
	distanza2g=distanza2g / US_ROUNDTRIP_CM;
	int distanza2 = (distanza2a+distanza2b+distanza2c+distanza2d+distanza2e+distanza2f+distanza2g)/7;
	int vardistanza2 = ((distanza2-distanza2a)^2+(distanza2-distanza2b)^2+ (distanza2-distanza2c)^2+(distanza2-distanza2d)^2+ (distanza2-distanza2e)^2+(distanza2-distanza2f)^2+ (distanza2-distanza2g)^2)/7;
	...
	switch (a) {
		case '0': // prendere info
			...
			Serial.print(luce2);
			Serial.print(",");
			Serial.print(vardistanza1);
			Serial.print(",");
			Serial.print(distanza1);
			Serial.print(",");
			Serial.print(vardistanza2);
			Serial.print(",");
		...
		case '3': // Ricalcola
			...
			break;
		case '4': // ruota cestino di 90 gradi
			digitalWrite(motorpinDC,HIGH);
			delay(1000);
			digitalWrite(motorpinDC,LOW);
			Serial.println("sposta");
			break;
		case '5': // ruota cestino di 180 gradi
			digitalWrite(motorpinDC, HIGH);
			delay(2000);
			digitalWrite(motorpinDC, LOW);
			Serial.println("sposta");
			break;
		case '6': // ruota cestino di 270 gradi
			digitalWrite(motorpinDC, HIGH);
			delay(3700);
			digitalWrite(motorpinDC, LOW);
			Serial.println("sposta");
...
Public Class Form1
	...
	Private Sub Label1_Click(sender As Object, e As EventArgs) Handles Label1.Click
		Dim exeapth As String = Application.StartupPath
	Process.Start(exeapth)
...
def resnet(input_shape, depth, num_classes=10): #per l'addestramento della resnet su cestino/chatbot abbiamo messo 4
...
# Importare i modelli
resnetCestino = load_model('CNN_ResNet_epoch35cestino.hdf5')
classify_filename = 'CNN_ResNet_epoch45_WS.pkl'
classify_model_pkl = open(classify_filename, 'rb')
resnetWS = pickle.load(classify_model_pkl)
classify_filename = 'Ensemble_telegram.pkl'
classify_model_pkl = open(classify_filename, 'rb')
knn_gscv = pickle.load(classify_model_pkl)

# Importo media
cestino_mean = np.load('x_train_mean_cestino.npy')
ws_mean = np.load('x_train_mean_WS.npy')
colonne=['resne2_carta', 'resne2_indifferenziata', 'resne2_plastica','resne2_vetro', 'resne1_Carta', 'resne1_ Indifferenziato','resne1_ Latta', 'resne1_ Legno', 'resne1_ Metalli', 'resne1_ Plastica','resne1_ Polistirolo', 'resne1_ Tetra_Pak', 'resne1_ Umido','resne1_ Vetro']
...
Public Class Form3
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
		...
		For t As Integer = 1 To 1000
			testo =  + SerialPort1.ReadExisting
			dataArr = testo.Split(",")
			If dataArr.Length = 10 Then
				Exit For
			End If
		Next
		
		Dim exePath As String = Application.StartupPath()
		Dim path As String = exePath + "\image.jpg"
		PictureBox2.Image.Save(path, Imaging.ImageFormat.Jpeg)
		
		Dim arg As String = " " & ControlChars.Quote & exePath & ControlChars.Quote
		
		For Each tb As String In dataArr
			arg = arg & " " & ControlChars.Quote & tb & ControlChars.Quote
		Next
		
		Dim Command As String = ControlChars.Quote & exePath & "\send\python\App\Python\python.exe" & ControlChars.Quote & " " & ControlChars.Quote & exePath & "\send\analizza.py" & ControlChars.Quote & arg
		Dim ProcID = Shell(Command, vbHide)
		
		Dim pr As Process = Process.GetProcessById(ProcID)
		Dim ticks As Integer = 0
		Dim test As String = exePath & "\send\test.txt"
		...
		risultato = My.Computer.FileSystem.ReadAllText(exePath & "\send\test.txt")
		My.Computer.FileSystem.DeleteFile(exePath & "\send\test.txt")
		TextBox1.Text = risultato
		MessageBox.Show("Cestino in movimento!")
		My.Computer.FileSystem.DeleteFile(exePath & "\send\test.txt")
		If risultato = "plastica" Then
			SerialPort1.Write("1")
			Threading.Thread.Sleep(500)
			SerialPort1.Write("2")
		End If
		If risultato = "carta" Then
			SerialPort1.Write("4")
			Threading.Thread.Sleep(500)
			SerialPort1.Write("1")
			Threading.Thread.Sleep(500)
		...
		If risultato = "vetro" Then
			...
			SerialPort1.Write("5")
		End If
		If risultato = "indifferenziata" Then
			SerialPort1.Write("6")
			Threading.Thread.Sleep(500)
			SerialPort1.Write("1")
			Threading.Thread.Sleep(500)
...
try:
	...
	classify_model_pkl = open(classify_filename, 'rb')
	knn_gscv = pickle.load(classify_model_pkl)
	classify_filename = 'GBM_model_cestino.pkl'
	classify_model_pkl = open(classify_filename, 'rb')
	gbm = pickle.load(classify_model_pkl)
	
	# Importo media
	cestino_mean = np.load('x_train_mean_cestino.npy')
	ws_mean = np.load('x_train_mean_WS.npy')
	colonne = ['resne2_carta', 'resne2_indifferenziata', 'resne2_plastica',
	'resne2_vetro', 'resne1_Carta', 'resne1_ Indifferenziato',
	'resne1_ Latta', 'resne1_ Legno', 'resne1_ Metalli', 'resne1_ Plastica',
	'resne1_ Polistirolo', 'resne1_ Tetra_Pak', 'resne1_ Umido',
	'resne1_ Vetro', 'GBM_carta', 'GBM_indifferenziata', 'GBM_plastica',
	'GBM_vetro']
	img_data = []
	...
	lum=(val[3]+val[4])/2
	ultrasuoni=val[5]
	colore=(val[7]+val[8]+val[9])/3
	df=pd.DataFrame(columns=['Peso', 'ultrasuoni1', 'colore', 'luminosità'])
	df.loc[0]=[peso,ultrasuoni,colore,lum]
	# Prediction
	## resnet1
	# categories=['Carta', 'Indifferenziato', 'Latta', 'Legno', 'Metalli', 'Plastica', 'Polistirolo', 'Tetra_Pak', 'Umido', 'Vetro']
	pred2 = resnetWS.predict(img_data_WS)
	...
	pred.append(list(pred1[0])[3])
	## GBM
	pred0 = gbm.predict_proba(df)
	for t in range(len(pred0[0])):
		pred.append(list(round(pred0[0][t], 3) * 1000)
	X = pd.DataFrame(columns=colonne)
	X.loc[0] = pred
	predictions = knn_gscv.predict(X)
	...
	my_file = open(sys.argv[1] + "/send/test.txt", "w+")
	my_file.write(str(predictions[0]))
	my_file.close()

except:
	os.system('cmd /k "Msg * "Errore""')
	my_file = open(sys.argv[1] + "/send/test.txt", "w+")
	my_file.write("C'è stato un errrore inaspettato")
	my_file.close()
...
\addcontentsline{toc}{chapter}{Bibliografia} \markboth{Bibliografia}{} 
...
\bibitem{chiave10a} Christian Szegedy - Vincent Vanhoucke - Sergey Ioffe - Jonathon Shlens - Zbigniew Wojna (2015), Rethinking the Inception Architecture for Computer Vision, Google inc. - University College London.
...
\bibitem{chiave16} Leo Breiman (1996), Bagging Predictors, University of California - Berkeley.
\bibitem{chiave17}  Mark J. van der Laan - Eric C. Polley - Alan E. Hubbard (2007), Super Learner, University of California - Berkeley.
\bibitem{chiave18} Gareth James - Daniela Witten - Trevor Hastie - Robert Tibshirani (2017), An Introduction to Statistical Learning, Springr.
\bibitem{chiave19} Steffen Rendle (2010), Factorization Machines, Osaka University.
\bibitem{chiave20} Willi Richert - Luis Pedro Coelho (2013), Building Machine Learning Systems with Python, Packt Publishing.
\bibitem{chiave21} Ethem Alpaydın (2010), Introduction to Machine Learning, Massachusetts Institute of Technology.
\bibitem{chiave21} Shai Shalev-Shwartz - Shai Ben-David (2014), Understanding Machine Learning: From Theory to Algorithms,  Cambridge University Press.
