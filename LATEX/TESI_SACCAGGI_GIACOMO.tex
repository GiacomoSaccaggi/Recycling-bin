...
\caption{Script di programmazione 1.2: Individuare gli attributi nella struttura ad albero DOM}
\begin{lstlisting}[language=R]
...
La prima pagina denominata \virgolette{Form1:Home} serve per indirizzare l'utente verso la funzione di addestramento e reperimento dei dati o la sezione di riconoscimento e analisi dei dati, che si vedrà in modo approfondito nel capitolo 3, si è deciso inoltre di aggiungere, nella parte di sinistra, delle istruzioni così da rendere più chiaro l'utilizzo del software e lo scopo del progetto.
...
\begin{itemize}
	\item [1.] Si inizializza un albero che non esegue divisioni e che per ogni classe restituisce una probabilità associata pari a zero 0 $\hat{f}^0(x)=0$, si decide un $B \gg 0$ e il parametro di restringimento (shrinkage parameter) $\lambda>0$ in modo arbitrario;
...
$$\mbox{f1-score}= 2 \space * \space \frac{\mbox{Precision * Recall}}{\mbox{Precision + Recall}} $$
...
Il processo di filtraggio, come si può osservare nell'esempio in Figura 2.15, consiste nello scorrimento di un filtro convoluzionale su un'immagine e nella generazione di una versione filtrata dell'immagine.
...
\begin{itemize}
	...
	\item Padding (imbottitura): consente, come esemplificato in Figura 2.16, di modificare l'immagine di input riempiendola con pixel vuoti per garantire che l'immagine filtrata mantenga le stesse dimensioni dell'immagine di input originale.
	\begin{figure}[H]
	\center
...
In questa parte si andrà ad analizzare la ragione per cui le reti residue hanno successo e consentono l'aggiunta più layer senza danneggiare le prestazioni della rete. Si consideri una semplice rete neurale ($A$) senza rete residua, come mostrato in Figura 2.19. Quindi nella rete ($A$) l'ingresso $X$ viene inserito in una rete neurale (NN) per ricevere come output $A_1$. Ora, si consideri una rete più profonda ($B$) in cui un blocco residuo (con 2 livelli extra e una connessione salta) viene aggiunto nella rete precedente. Quindi ora $A_1$ passa al blocco residuo, che, a sua volta dà la nuova attivazione $A_3$.
...
L'applicazione di modelli sovrapposti ai problemi dei Big Data può produrre una maggiore accuratezza e robustezza delle previsioni rispetto ai singoli modelli. L'approccio dello stacking dei modelli sposta l'obbiettivo dalla modellizzazione e dalla ricerca del modello migliore a quello di trovare una raccolta di modelli complementari validi.
...
- spiegare modello di regressione e funzione di perdita per aumentare al massimo la specificità dell'accuratezza chissene
